# Bash Scripting Labs

This repository contains a collection of Bash scripts for hands-on lab exercises and challenge tasks.

## Project Contents

- `Challenge1_menu_tool.sh` - An interactive menu-driven script to demonstrate Bash menu options and user interaction.
- `Challenge3_infrastructure_setup.sh` - A script for infrastructure setup tasks, likely provisioning or configuration automation.
- `Lab1_nginx_recovery.sh` - A recovery script for restoring or restarting an Nginx service.
- `Lab3_ec2_monitoring.sh` - A monitoring script intended for AWS EC2 instances.

## Screenshots

Screenshots are stored in the `screenshots/` folder.

### [Lab1_nginx_recovery.sh](Lab1_nginx_recovery.sh)
![Nginx recovery screenshot](screenshots/Screenshot%20From%202026-06-02%2022-05-30.png)

### [Lab3_ec2_monitoring.sh](Lab3_ec2_monitoring.sh)
![EC2 monitoring screenshot](screenshots/Screenshot%20From%202026-06-02%2022-14-31.png)

### [Challenge1_menu_tool.sh](Challenge1_menu_tool.sh)
![Menu tool screenshot - Part 1](screenshots/Screenshot%20From%202026-06-02%2022-17-18.png)

![Menu tool screenshot - Part 2](screenshots/Screenshot%20From%202026-06-02%2022-18-13.png)

### [Challenge3_infrastructure_setup.sh](Challenge3_infrastructure_setup.sh)
![Infrastructure setup screenshot - Step 1](screenshots/Screenshot%20From%202026-06-02%2022-19-22.png)

![Infrastructure setup screenshot - Package downloads](screenshots/Screenshot%20From%202026-06-02%2022-19-30.png)

![Infrastructure setup screenshot - Final steps](screenshots/Screenshot%20From%202026-06-02%2022-23-11.png)

## Usage

1. Open the script you want to run.
2. Make the script executable if necessary: `chmod +x <script-name>.sh`
3. Run the script: `./<script-name>.sh`

## Notes

- The screenshots are referenced relative to this README and should render correctly on GitHub or other Markdown viewers.
- Adjust the descriptions above if you want to match the exact behavior of each script more closely.
